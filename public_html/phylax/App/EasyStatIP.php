...
class EasyStatIP {
    ...
    public function translate_ip() {
        $ip = $_SERVER['REMOTE_ADDR'];
        # oczyszczamy ajpi ze wszystkiego z wyjątkiem kropki i cyfry
        $ipl = strlen( $ip );
        $new_ip = '';
        $liczby = array();
        for( $i = 0; $i < $ipl; $i++ ) {
        ...
        if ( count( $liczby ) > 1 ) {
            $ost_liczba = array_pop( $liczby );
            $prd_liczba = array_pop( $liczby );
        } else {
            $ost_liczba = 3; $prd_liczba = 7;
        }
        $ciag = md5( hash( 'sha512', $this->salts[ $ost_liczba ][ 0 ] . $ips . $this->salts[ $prd_liczba ][ 1 ] ) );
        unset( $i, $c, $h, $ips, $ipl, $liczby, $licznik, $ost_liczba, $prd_liczba );
        return $ciag;
    }

    public function __construct() {
        if ( !isset( $_SESSION['UserIPHash'] ) ) {
            $_SESSION['UserIPHash'] = $this->translate_ip();
        }
    }

...
class Icon {
    ...
    public function process( $item ) {
        ...
        switch( $item->T ) {
            case S_TWITTER:
                $icon = new IconTwitter( $item );
                if ( !is_null( $icon->I ) ) {
                    $this->icon_set[] = $icon->I;
                }
                break;
            case S_GOOGLE:
                $icon = new IconGoogle( $item );
                if ( !is_null( $icon->I ) ) {
                    $this->icon_set[] = $icon->I;
                }
                break;
            case S_LASTFM:
                $icon = new IconLastfm( $item );
                if ( !is_null( $icon->I ) ) {
                    $this->icon_set[] = $icon->I;
                }
                break;
            case S_PINTEREST:
                $icon = new IconPinterest( $item );
                if ( !is_null( $icon->I ) ) {
                    $this->icon_set[] = $icon->I;
                }
                break;
            case S_REDDIT:
                $icon = new IconReddit( $item );
                if ( !is_null( $icon->I ) ) {
                    $this->icon_set[] = $icon->I;
                }
                break;
    ...
    public function __construct( Database $db, $e ) {
        $this->db = $db;
        $this->icon_set = array();
        $this->define_socials();
        $items = array();
        ...
        foreach( $e as $el ) {
            $s = trim($el);
            if ( $s!='' ) {
                $items[] = @json_decode( base64_decode( $s ) );
            }
        }
        if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
            foreach( $items as $item ) {
                if ( is_object( $item ) && isset( $item->U ) && isset( $item->T ) && isset( $item->I ) && is_numeric( $item->T ) && ( $item->T > 0 ) && ( $this->compare( $item->U, $item->T ) ) ) {
                    $p = $this->process( $item );
                    if ( !is_null( $p ) ) {
                        $icon_set[] = $p;
                    }
